...
function doGet(e) {
  try {
    ...
    // Handle POST-as-GET: payload param contains JSON body
    if (params.payload) {
      const body = JSON.parse(decodeURIComponent(params.payload));
      const result = dispatch_post(body.action || '', body);
      return json(result);
    }

    // Normal GET with action param
    const action = params.action || 'dashboard';
    const result = dispatch_get(action);
    return json(result);
  } catch(err) {
